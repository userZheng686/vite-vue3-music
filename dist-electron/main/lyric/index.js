...
let win;
const createLyricWindow = function(BrowserWindow2) {
...
import_electron.ipcMain.on("lyricHide", () => {
  win.hide();
});
import_electron.ipcMain.on("lyricShow", () => {
  win.show();
  win.focus();
});
import_electron.ipcMain.handle("lyricGetBounds", (event) => {
  return win.getBounds();
});
import_electron.ipcMain.handle("lyricSetBounds", (event, param) => {
...
import_electron.ipcMain.handle("lyricSetSize", (event, param) => {
...
var stdin_default = createLyricWindow;
