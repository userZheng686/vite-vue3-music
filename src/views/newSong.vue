<template>
  <section class="new--song">
    ...
    <el-button
      ...
      :icon="VideoPlay"
      type="danger"
      @click="play(newWorksListAll, newWorksListAll[0].id)"
    >
      播放最新50
    </el-button>
    <NewSongListItem v-for="item in newWorksList" :key="item.publishTime" :item="item" />
    <el-button
      v-if="isMore && newWorksList.length"
      class="button"
      type="danger"
      @click="getArtistNewSongList()"
    >
      加载更多</el-button
    >
  </section>
...
let getArtistNewSongList = async () => {
  try {
    let {
      data: { hasMore, newWorks, latestVisitTime },
    } = await getArtistNewSong(startTimestamp, 1, 10, firstRequest);
    let {
      data: { songList },
    } = await getArtistNewSongPlayall();
