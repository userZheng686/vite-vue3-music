import { ColumnSubListItem } from "i/api/api_column";
import { useColumnShare } from "@/utils/dialogShare";
import { menusEvent } from "vue3-menus";
import { handleCopyLink, loginCallback } from "../utils";

//播客的右键菜单
export async function contextMenuColumn(event: MouseEvent, item: ColumnSubListItem) {
    ...
    let list = [
        ...
        },
        {
            ...
            click: () => {
                loginCallback(() => {
                    useColumnShare(item.id,item.title,item.rectanglePicUrl)
                })
            },
        },
        {
