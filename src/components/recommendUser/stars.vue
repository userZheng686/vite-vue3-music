<template>
    <section class="star">
        <div class="star--list">
            <RecommendUserListItem v-for="item in list" :key="item.userId" :item="item" :SubscribedCount="false" :detailDescription="false" />
        </div>
        <el-button v-if="isMore && list.length" style="margin: 10px auto; display: flex; align-items: center"
            type="danger" @click="getStarList()">加载更多</el-button>
    </section>
</template>
...
let getStarList = async () => {
    try {
        let { celebrities, more } = await getUserRecommendCelebrities(2, offset, 60);
