<template>
  <section>
    <tableEmpty v-if="count === 0 && !loading">
      <template #empty
        >未能找到与<span class="like">"{{ keyword }}"</span>相关的任何歌词</template
      >
    </tableEmpty>
    <template v-else>
      <div class="info">找到{{ count }}首歌词</div>
      <CustomElementTable
        :list="songList"
        :index="true"
        :isShowLabel="true"
...
let getSongList = async () => {
  try {
    ...
    } = await getSearch(String(route.query.keyword), 1006, 50, offset);
