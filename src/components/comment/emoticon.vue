...
let closeEmotion = () => {
  if (sharePopup.childSpaceWindowShow) {
    ...
    sharePopup.privateLetterEmotion = false;
  } else {
...
let updateTextarea = (meaning: string) => {
  if (sharePopup.childSpaceWindowShow || sharePopup.childPrivateLetterShareShow) {
    if (sharePopup.shareContent.length + meaning.length >= 140) return;
    sharePopup.shareContent += `[${meaning}]`;
  } else {
    if (sharePopup.content.length + meaning.length >= 140) return;
    sharePopup.content += `[${meaning}]`;
  }
};
</script>
